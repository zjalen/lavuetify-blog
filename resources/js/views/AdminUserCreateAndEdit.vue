...
<script>
import { createAdminUser, getAdminUser, updateAdminUser } from '../api/admin-user'

export default {
  name: 'AdminUserCreateAndEdit',
  data:() => ({
    item: {},
    avatar: null,
    image_url: null,
  ...
  mounted () {
    if (this.$route.params.id) {
      getAdminUser(this.$route.params.id).then(response => {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if (this.item.password) params.append('password', this.item.password)
      params.append('name', this.item.name)
      if (this.avatar) params.append('avatar', this.avatar)
      if (this.item.id) {
        params.append('_method', 'PUT')
        updateAdminUser(this.item.id, params).then(() => {
